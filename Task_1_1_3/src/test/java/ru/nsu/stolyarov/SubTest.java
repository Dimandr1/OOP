...
class SubTest {
    ...
    void test() {
        ...
        assertEquals("(x-20.0)", testSub.print());
        assertEquals(-50, testSub.eval("x = -30"));
        testExpr = testSub.derivative("x");
        assertEquals(1, testExpr.eval("x = 5"));
        assertEquals("(1.0-0.0)", testExpr.print());
        ...
        assertEquals("(0.0-1.0)", testExpr.print());
    }
}